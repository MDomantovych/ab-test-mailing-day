...
**Recommendation.** Friday as a temporary default; run a clean 2×2 test (day × hour) randomized by user with a 10 % holdout,
primary metric = return to activity within 7–14 days, CTR secondary.
...
> holdout-групи немає. Рішення: п'ятниця як тимчасовий дефолт + чистий тест 2×2 з holdout і метрикою повернення активності.
...
* **Стійкість:** z-тест, OLS з кластерними SE по користувачу, кластерний бутстреп, поправка на канал/воркфлоу/тиждень,
  порівняння всередині користувача (fixed effects) і парне порівняння по 12 тижнях дають +1.4…+1.8 п.п.
  Порівняння всередині користувача прибирає між-користувацьку дисперсію, тобто виконує роль CUPED без окремого передперіоду.
...
* дизайн 2×2: {вівторок, п'ятниця} × {14:00, 16:00}, щоб розділити день і час;
...
* тривалість: MDE 1 п.п. по CTR ≈ 9 тис. відправок на гілку — ~7 тижнів для двох гілок, ~13 для чотирьох; зупинка за планом.
...
├── tests/test_abtools.py            <- 11 тестів: чисті функції на відомих значеннях + інтеграційний тест на реальному лозі
